...
import { TaskForm, TaskFormMethods } from '@/components/task-form';
...
export const TaskListItem = ({ task, provided, form }: TaskListItemProps) => {
  const updateTaskMutation = useUpdateTaskMutation(task.projectId, task.id);
  ...
  }, [task.id, updateTaskMutation, form, task.projectId, task.status]);
